...
const AlgemeenTab: React.FC = () => {
  ...
  return <div className="space-y-6 sm:space-y-8 animate-slide-up">
      <section>
        <h2 className="text-xl sm:text-2xl font-semibold mt-8 px-1 my-[30px]">Over de Competitie 15u42</h2>
        <Card>
      ...
      <section>
        <h2 className="text-xl sm:text-2xl font-semibold mt-8 px-1 my-[30px]">Laatste Nieuws</h2>
          ...
          {loading ? <p className="text-center text-sm sm:text-base py-8">Berichten laden...</p> : blogPosts.length > 0 ? blogPosts.map(post => <Card key={post.id} className="card-hover w-full">
                ...
                <CardContent className="bg-transparent ">
                  ...
                  {post.tags && post.tags.length > 0 && <div className="flex flex-wrap gap-2 mt-3">
                      {post.tags.map((tag, index) => <Badge key={index} className="badge-purple-white">{tag}</Badge>)}
                    </div>}
                </CardContent>
      ...
      <section>
        <h2 className="text-xl sm:text-2xl font-semibold mt-8 px-1 my-[30px]">Contact</h2>
        <Card>
