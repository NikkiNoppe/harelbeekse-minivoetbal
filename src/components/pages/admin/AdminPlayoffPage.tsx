...
import { 
  Loader2, Trophy, AlertCircle, Trash2, Calendar, CheckCircle, Clock, Undo2, 
...
const WorkflowStepper = ({ status }: { status: PlayoffStatus }) => {
  const steps = [
    { id: 'none', label: 'Configureer', shortLabel: '1', icon: Settings },
    { id: 'concept', label: 'Concept', shortLabel: '2', icon: Clock },
    { id: 'finalized', label: 'Definitief', shortLabel: '3', icon: CheckCircle }
  ];
  ...
  return (
    <div className="flex items-center justify-center gap-1 sm:gap-2 p-3 bg-muted/30 rounded-lg">
      {steps.map((step, i) => {
        ...
        return (
          <React.Fragment key={step.id}>
            <div className={cn(
              "flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-full text-xs sm:text-sm transition-colors",
              state === 'current' && "bg-[hsl(var(--color-500))] text-white",
              state === 'completed' && "bg-[hsl(var(--color-100))] text-[hsl(var(--color-700))]",
...
const MobileStandingsCard = ({ 
  ...
  <div className={cn(
    ...
    isTopTeam && "bg-green-50 dark:bg-green-950/30",
    isBottomTeam && "bg-amber-50 dark:bg-amber-950/30",
    !isTopTeam && !isBottomTeam && "bg-muted/50"
  )}>
    <div className="flex items-center gap-3 min-w-0">
      <span className={cn(
        "w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0",
        isTopTeam && "bg-green-200 text-green-800 dark:bg-green-800 dark:text-green-100",
        isBottomTeam && "bg-amber-200 text-amber-800 dark:bg-amber-800 dark:text-amber-100",
        !isTopTeam && !isBottomTeam && "bg-muted-foreground/20 text-muted-foreground"
      )}>
        {team.position}
      </span>
    ...
    <div className="flex items-center gap-2 flex-shrink-0">
      ...
      {isTopTeam && (
        <Badge variant="outline" className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100 text-[10px] px-1.5">
          Top
        </Badge>
      )}
      {isBottomTeam && (
        <Badge variant="outline" className="bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-100 text-[10px] px-1.5">
          Bottom
        </Badge>
      )}
    </div>
...
const MobileMatchCard = ({ 
  ...
  <div className="p-3 bg-muted/50 rounded-lg">
    <div className="flex items-center justify-between mb-2">
      ...
      {!match.is_playoff_finalized && (
        <Badge variant="outline" className="text-[10px] h-5">Concept</Badge>
      )}
    </div>
    <div className="flex items-center gap-2">
      <div className="flex-1 text-right min-w-0">
        <span className={cn(
          ...
          match.is_playoff_finalized ? "font-medium" : "text-muted-foreground italic"
        )}>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {standings.map((team) => (
                  <MobileStandingsCard
                    key={team.team_id}
                    team={team}
                    isTopTeam={team.position <= topTeamCount}
                    isBottomTeam={team.position > topTeamCount && team.position <= topTeamCount + bottomTeamCount}
                  />
                ))}
              </div>
            ) : (
              ...
              <div className="grid grid-cols-2 gap-3">
                <Card className="bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800">
                  <CardHeader className="p-3 pb-2">
                    <CardTitle className="text-xs sm:text-sm">Top Preview</CardTitle>
                  </CardHeader>
                  <CardContent className="p-3 pt-0 text-xs sm:text-sm space-y-0.5">
                    {Array.from({ length: Math.min(4, topTeamCount / 2) }, (_, i) => (
                      <div key={i} className="flex justify-between text-muted-foreground">
                        <span>{i + 1}e</span>
                        <span>vs</span>
                        <span>{topTeamCount - i}e</span>
                      </div>
                    ))}
                  </CardContent>
                </Card>
                <Card className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800">
                  <CardHeader className="p-3 pb-2">
                    <CardTitle className="text-xs sm:text-sm">Bottom Preview</CardTitle>
                  </CardHeader>
                  <CardContent className="p-3 pt-0 text-xs sm:text-sm space-y-0.5">
                    {Array.from({ length: Math.min(3, Math.floor(bottomTeamCount / 2)) }, (_, i) => (
                      <div key={i} className="flex justify-between text-muted-foreground">
                        <span>{topTeamCount + i + 1}e</span>
                        <span>vs</span>
                        <span>{topTeamCount + bottomTeamCount - i}e</span>
                      </div>
                    ))}
                  </CardContent>
                </Card>
          ...
          <Card className="border-amber-200 dark:border-amber-800">
            <CardContent className="p-4 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                >
                  {actionLoading && confirmAction === 'finalize' ? (
                ...
                <Button
                  ...
                  disabled={actionLoading}
                  className="h-11 text-destructive border-destructive hover:bg-destructive/10"
                >
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <Button
                  ...
                  className="flex-1 h-11"
                >
                  {actionLoading && confirmAction === 'unfinalize' ? (
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  ) : (
        ...
        <AlertDialogContent className="max-w-[90vw] sm:max-w-md">
          ...
          <AlertDialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <AlertDialogAction 
              ...
              className={cn(
                "h-11 sm:h-10",
                confirmAction === 'delete' && "bg-destructive hover:bg-destructive/90"
              )}
            >
              {actionLoading ? (
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
