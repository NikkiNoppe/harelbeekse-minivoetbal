        ...
        <div className="container flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={handleLogoClick} role="button" aria-label="Go to home page">
            <div className="h-12 w-12 bg-white rounded-full flex items-center justify-center shadow-md">
              <svg className="h-8 w-8 text-soccer-green" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const TabItem: React.FC<TabItemProps> = ({
  ...
  return <TabsTrigger value={value} className={cn("flex-1 min-w-max flex items-center justify-center gap-2 px-4 py-2 font-medium", "data-[state=active]:bg-white data-[state=active]:text-soccer-green data-[state=active]:shadow-sm", "text-gray-600 transition-all")}>
      {icon}
      <span>{label}</span>
    </TabsTrigger>;
};
