...
import { User } from "../../../../MINIVOETBAL.MODEL/auth";
...
export const useLogin = (onLoginSuccess: () => void) => {
  ...
  const login = async (usernameOrEmail: string, password: string) => {
    ...
    try {
      ...
      console.log('❌ Verification error:', error, 'Status:', status);

      if (error) {
        console.error('💥 Database error during password verification:', error, 'Status:', status);
        if (error.code === '42501' || error.message?.toLowerCase().includes('permission')) {
          toast({
            title: "Login mislukt",
            description: `Geen toegang tot login. Controleer Supabase policies/RLS. (${error.message})`,
            variant: "destructive",
          });
          return;
        }
      ...
      toast({
        title: "Login mislukt",
        description: `Er is een onverwachte fout opgetreden tijdens het inloggen: ${(error as any)?.message || error}`,
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
