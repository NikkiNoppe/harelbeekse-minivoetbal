-- Profile polls: tenant-isolatie via organization_id + SuperAdmin acting_organization_id in sessie.

-- SuperAdmin: organization_id uit acting_organization_id (niet NULL)
...
-- RLS: profile_polls terug in read-policy (per tenant)
...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Geen organisatie in sessie');
  END IF;
...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Geen organisatie in sessie — kies eerst een tenant (SuperAdmin)');
  END IF;
