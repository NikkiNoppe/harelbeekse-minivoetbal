-- Wedstrijdcontext: tegenstander-contact voor team managers; beide teams voor admin/scheids.
...
BEGIN
  ...
  IF v_role = 'player_manager' THEN
    ...
    WHERE t.organization_id = v_org_id
      AND t.team_id IN (p_home_team_id, p_away_team_id)
      AND NOT (t.team_id = ANY(v_team_ids))
...
COMMENT ON FUNCTION public.get_match_teams_contact_for_session(uuid, integer, integer) IS
  'Wedstrijdformulier: admin/scheids ziet beide teams; team manager alleen tegenstander-contact binnen eigen wedstrijd.';
