-- Fix delete_team_costs_for_match by removing SECURITY DEFINER
-- This allows the function to execute within the trigger context
-- so that pg_trigger_depth() > 0 check in RLS policy works correctly
...
RETURNS void
LANGUAGE plpgsql
-- SECURITY DEFINER removed to allow trigger context to pass through
