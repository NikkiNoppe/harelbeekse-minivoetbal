...
ON public.users 
FOR UPDATE 
USING (auth.uid()::integer = user_id OR public.get_current_user_role() = 'admin');
...
CREATE POLICY "Team managers can read full team data" 
ON public.teams 
FOR SELECT 
USING (
...
-- No read policy for anon - tokens should only be validated server-side
