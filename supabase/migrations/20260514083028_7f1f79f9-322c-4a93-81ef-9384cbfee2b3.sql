...
CREATE OR REPLACE FUNCTION public.admin_set_referee_availability(p_admin_user_id integer, p_referee_id integer, p_match_id integer, p_poll_group_id text, p_poll_month text, p_is_available boolean, p_notes text DEFAULT NULL::text)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF p_is_available IS NULL THEN
    ...
    WHERE referee_id = p_referee_id
      AND is_available IS NULL AND assigned_at IS NULL
      AND ((p_match_id IS NOT NULL AND match_id = p_match_id)
        OR (p_match_id IS NULL AND poll_group_id = p_poll_group_id AND poll_month = p_poll_month));
...
CREATE OR REPLACE FUNCTION public.assign_referee_to_match(p_user_id integer, p_match_id integer, p_referee_id integer, p_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  VALUES (p_referee_id, p_match_id, p_user_id, now(), v_poll_month)
  ON CONFLICT (referee_id, match_id) WHERE match_id IS NOT NULL
...
CREATE OR REPLACE FUNCTION public.assign_referee_to_session(p_user_id integer, p_match_id integer, p_referee_id integer, p_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  LOOP
    IF NOT EXISTS (
      ...
      INSERT INTO referee_matches (referee_id, match_id, assigned_by, assigned_at, poll_month)
      VALUES (p_referee_id, v_session_match.match_id, p_user_id, now(), v_poll_month)
      ON CONFLICT (referee_id, match_id) WHERE match_id IS NOT NULL
      DO UPDATE SET assigned_by = EXCLUDED.assigned_by,
                    assigned_at = EXCLUDED.assigned_at;
...
BEGIN
  ...
  LOOP
    ...
    SET assigned_by = NULL, assigned_at = NULL
    WHERE match_id = v_session_match.match_id AND assigned_at IS NOT NULL;

    DELETE FROM referee_matches
    WHERE match_id = v_session_match.match_id AND is_available IS NULL AND assigned_at IS NULL;
