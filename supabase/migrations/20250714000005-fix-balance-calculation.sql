-- Fix balance calculation to properly handle all costs and deposits
-- Update the balance calculation function to use individual amounts and correct categories

-- Step 1: Update the balance calculation function to handle all transaction types correctly
...
-- Step 2: Create a detailed financial breakdown function
CREATE OR REPLACE FUNCTION public.get_team_financial_breakdown(team_id_param INTEGER)
RETURNS TABLE (
...
BEGIN
  ...
  WITH team_transactions AS (
    SELECT 
      tc.amount as transaction_amount,
      cs.category,
      cs.name,
      tc.transaction_date
    FROM public.team_costs tc
...
-- Step 3: Update all team balances to use the new calculation
UPDATE public.teams 
SET balance = public.calculate_team_balance_updated(team_id);

-- Step 4: Create a view for financial overview
...
SELECT 
  ...
  fb.adjustments,
  fb.current_balance,
  t.balance as calculated_balance
FROM public.teams t
CROSS JOIN LATERAL public.get_team_financial_breakdown(t.team_id) fb
ORDER BY t.team_name;

-- Step 5: Add RLS policy for the financial overview
CREATE POLICY "Admins can view team financial overview" 
ON public.team_financial_overview 
FOR SELECT 
USING (true); 