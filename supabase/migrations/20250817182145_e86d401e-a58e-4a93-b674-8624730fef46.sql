...
CREATE POLICY "Only admins can write application settings" 
ON public.application_settings 
FOR INSERT
WITH CHECK (get_current_user_role() = 'admin'::text);

CREATE POLICY "Only admins can update application settings" 
ON public.application_settings 
FOR UPDATE
USING (get_current_user_role() = 'admin'::text)
WITH CHECK (get_current_user_role() = 'admin'::text);

CREATE POLICY "Only admins can delete application settings" 
ON public.application_settings 
FOR DELETE
USING (get_current_user_role() = 'admin'::text);
